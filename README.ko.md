...
<!-- VIRUSTOTAL: 이 릴리스 gsm.exe의 VirusTotal 리포트 URL을 여기에 붙여넣기, 예 — [VirusTotal 리포트](https://www.virustotal.com/gui/file/<hash>) -->
...
| Soulmask | |
| ARK: Survival Ascended | 맵 선택·난이도·배율, RCON 콘솔 지원. 서버마다 따로 설치(약 13GB). CurseForge 모드는 아직 미지원 |
| Conan Exiles | PvP/PvE·노출 수위·경험치 배율, RCON 콘솔 지원. 서버마다 따로 설치(약 6GB). 스팀 워크샵 모드는 아직 미지원 |

마지막 7종은 **Windows 전용** 데디케이티드 서버만 존재합니다 (리눅스 빌드에서는 숨김).
