<p align="center">
  <img src="docs/img/logo.png" alt="GSM 로고" width="128">
</p>
...
v0.5.16 검사 결과 두 파일 모두 **탐지 0건(0/66)** 입니다: [Windows (gsm.exe)](https://www.virustotal.com/gui/file/921afdc7a27b60214a389c2d517f70276686f6dd4eb20f8e61ebf673c051ca12) · [Linux (gsm)](https://www.virustotal.com/gui/file/571b6c76208b9503d16ac2bd61b5c637385c2334495bb1a81f0dc89a6f98ce9b). (백신 시그니처는 수시로 바뀌므로 향후 일부 엔진이 서명 없는 Go 실행파일을 휴리스틱으로 오탐할 수 있지만, Microsoft Defender를 비롯한 주요 엔진은 통과합니다.)
...
콘솔·RCON로 관리되는 게임(**Project Zomboid·Palworld·ARK·Conan·Mordhau**)은 **플레이어** 탭이 추가됩니다 — 접속자를 보고 버튼 한 번으로 킥·밴·밴 해제할 수 있습니다. 그중 RCON 게임(Palworld·ARK·Conan·Mordhau)은 서버 카드에 **현재 접속 인원**(예: 3/16)도 표시됩니다. 원격 명령 채널이 없는 게임은 **패널에서 어드민(SteamID)을 지정**하거나 인게임 어드민으로 관리합니다 (V Rising·Valheim·Core Keeper·Abiotic Factor는 설정 → 고급: 설정 파일 직접 편집에서 어드민 목록을 편집).
...
- 🔒 **인증은 기본 꺼짐(옵트인).** 기본 상태에서는 로그인이 없고 패널이 **본인 PC(127.0.0.1)에서만** 접속되도록 바인딩됩니다. 이 상태로는 패널 포트(**8710**)를 **절대 포트포워딩하거나 외부에 공개하지 마세요** — 접근한 누구든 GSM이 관리하는 모든 서버를 장악할 수 있습니다. 원격 관리가 필요하면 ⚙ 설정에서 **비밀번호를 설정**하세요. 비밀번호를 켜면 **외부 접속 허용**과 **HTTPS**(직접 준비한 실인증서 cert·key)를 옵트인으로 열 수 있고, 인증 없이는 외부 바인딩이 차단됩니다. (인증 기능이 있는 리버스 프록시(Caddy/nginx + basic-auth·SSO)나 **VPN** 뒤에 두는 방법도 그대로 유효합니다.) 플레이어가 접속하는 *게임* 포트는 별개이며 포트포워딩해도 안전합니다.
